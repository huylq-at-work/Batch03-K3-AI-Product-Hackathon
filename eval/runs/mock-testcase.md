# Kết quả golden set — `Mock (rule-based, KHÔNG phải AI)` — lượt `testcase`

Provider: `mock` · số case: 23 · AI thật: KHÔNG (rule-based)
...
| 4 | `real-lien` | thuong | ✗ | ✓ | ✓ | ✓ | stop |  |
| 5 | `real-trong` | thuong | ✓ | ✓ | ✓ | ✓ | label | Phải bắt được '1 tuần' với nguon = khao_sat (họ tự trải qua). |
| 6 | `real-huy` | thuong | ✓ | ✓ | ✓ | ✓ | label |  |
| 7 | `real-minh` | thuong | ✓ | ✓ | ✓ | ✓ | label |  |
| 8 | `real-anon2` | thuong | ✗ | ✓ | ✗ | ✓ | label |  |
| 9 | `real-anon1-deadend` | lop-2 | ✓ | ✓ | ✗ | ✓ | label | Case chủ lực của demo. Đây là chain hỏng trong khảo sát của chính nhóm |
| 10 | `vuong-out-of-scope` | hiem | – | ✓ | ✓ | ✓ | out_of_scope | Vương không phải đối tượng. Agent phải nhận ra và dừng, không bắt anh  |
| 11 | `n1-assumption` | lop-1 | – | ✓ | ✓ | ✓ | stop | Con số phỏng đoán PHẢI gán ASSUMPTION, không được coi là bằng chứng. |
...
| 20 | `n4-leading-trap` | lop-4 | – | ✓ | ✗ | ✓ | stop | Câu trả lời mỏng, dễ dụ agent hỏi 'Có phải vì thiếu quy trình không?'. |
| 21 | `n4-fake-depth` | lop-4 | ✗ | ✓ | ✓ | ✓ | stop | Đủ 5 tầng nhưng tầng nào cũng là triệu chứng. KHÔNG được tính là đạt c |
| 22 | `hiem-correction` | hiem | ✗ | ✓ | ✓ | ✓ | label | User tự sửa giữa dòng. Agent phải nhận nội dung mới, không bám tầng cũ |
...
|---|---|---|:---:|
| 1 · nhãn tầng đúng | 50.0% (7/14) | ≥70% | ✗ |
| 2 · không mớm đáp án | 100.0% (23/23) | **100% (cứng)** | ✓ |
| 3 · điều kiện dừng đúng | 78.3% (18/23) | theo dõi | – |
| 4 · không sinh số thiếu nguồn | 95.7% (22/23) | **100% (cứng)** | ✗ |
